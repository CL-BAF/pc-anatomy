...
| `tests/*.test.ts` | 62 tests: catalogue integrity, layout, picking, geometry presence, airflow, the RAM dive. |
...
62 tests, all through Node's built-in runner.
...
- `tests/ram.test.ts` (10) — the Memory menu order and the open chain down to
  ...
  the package stack order, named geometry on every new scale, and search
